...
namespace Autores.Data
{
    ...
    sealed public class CPFAttribute : ValidationAttribute
    {
        ...
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            ...
            if (validationContext.ObjectInstance.GetType() == typeof(Autor))
            {
                var model = (Autor)validationContext.ObjectInstance;

                if (context.Usuarios.Any(p => (p.CPF == CPF) && (p.Id != model.Id)))
                {
